...
function initMap() {
  ...
  const initialLatLng = [parseFloat(latInput.value), parseFloat(lonInput.value)];
...
$("run").onclick = async () => {
  ...
  try {
    ...
    $("kTemp").textContent = isFinite(data.climate.tempC) ? data.climate.tempC.toFixed(1) : "—";
    $("kRain").textContent = isFinite(data.climate.annualRainMm) ? data.climate.annualRainMm.toFixed(0) : "—";
    $("kSolar").textContent = isFinite(data.climate.solarMJm2day) ? data.climate.solarMJm2day.toFixed(1) : "—";
    $("kRH").textContent = isFinite(data.climate.rhPct) ? data.climate.rhPct.toFixed(0) : "—";
    $("kWind").textContent = isFinite(data.climate.windMps) ? data.climate.windMps.toFixed(1) : "—";
    $("kPh").textContent = (data.soil.ph != null && isFinite(data.soil.ph)) ? data.soil.ph.toFixed(1) : "—";
    $("kClay").textContent = (data.soil.clay != null && isFinite(data.soil.clay)) ? data.soil.clay.toFixed(0) : "—";
    $("kSilt").textContent = (data.soil.silt != null && isFinite(data.soil.silt)) ? data.soil.silt.toFixed(0) : "—";
    $("kSand").textContent = (data.soil.sand != null && isFinite(data.soil.sand)) ? data.soil.sand.toFixed(0) : "—";
    ...
    const sketch = function(p) {
      ...
      p.drawPlant = function(frame) {
        ...
        const plantColor = alive ? p.lerpColor(deadColor, healthyColor, overallHealth) : deadColor;
